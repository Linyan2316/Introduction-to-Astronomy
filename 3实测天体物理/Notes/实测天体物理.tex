...
利用开普勒第三定律
\begin{equation}
\frac{\alpha^{3}}{T^{2}}=\frac{\symup{G}}{4\pi^{2}}\left(M_{1}+M_{2}\right)
\end{equation}
...
相对口径：$A=D/f$. 倒数为焦比。面源成像面积正比于 $f^{2}$, 光通量 $\propto{}D^{2}$, 因此单位面积光强正比于 $A^{2}$. 相对口径 A 越大，就越能看到更暗弱的面源天体。实际的 A 值往往受到像差的限制而不能随意取值。折射 $A\sim\dfrac{1}{7}\text{\textendash}\dfrac{1}{5}$, 反射 $\sim\dfrac{1}{5}\text{\textendash}\dfrac{1}{3}$, 折反射 $\sim\dfrac{1}{3}\text{\textendash}\dfrac{1}{2}$, 甚至可以大于 1.
...
\begin{equation}
f'=f_{\text{抛}}\frac{\text{双曲面到 F1}}{\text{双曲面到 F2}}.
\end{equation}
...
\begin{equation}
P_{\lambda}\left(z\right)=\symup{e}^{-\tau_{\lambda}\left(z\right)}=\symup{e}^{-\tau_{\lambda}\left(0\right) M^{\left(z\right)}}=\left(P_{\lambda}\left(0\right)\right)^{M\left(z\right)}.
\end{equation}
而天体辐射在大气内外的辐射流之比为 $\dfrac{F_{\lambda}}{F_{0,\lambda}}=P_{\lambda}\left(z\right)$, 因此大气内外星等差为
\begin{equation}
\laplacian{}m=m_{\lambda}-m_{0,\lambda}=-2.5\lg\frac{F_{\lambda}}{F_{0,\lambda}}=-2.5M\left(z\right)\lg P_{\lambda}\left(0\right).
\end{equation}
