...
\noindent An $\Omega=1$ Universe that contains only a false vacuum expands exponentially as $R\mleft(t\mright)/R_{0}=\exp\mleft\{H_{0}\mleft(t-t_{0}\mright)\mright\}$ and is spatially flat with $k=0$. It is often called `de Sitter space'.

\hspace*{\fill}

\noindent\textcolor{purple}{(a)} Derive an expression for $s\mleft(\omega\mright)$ to an object at redshift $z$. This is the quantity that is used in calculating the brightnesses and angular sizes of objects in such a Universe.

\noindent Hint: You can use the following equation:
\begin{equation}
\symup{c}\d{}t=-R\mleft(t\mright)\d{}\omega\tag{1}
\end{equation}
to determine $\d{}\omega/\d{}z$ and thus $\omega\mleft(z\mright)$. Also, for a spatially flat universe $s\mleft(\omega\mright)=\omega$. Finally, in a flat geometry the angles of triangles are well-behaved.

\hspace*{\fill}

\noindent\textcolor{purple}{Solution}
\begin{align}
R\mleft(t\mright)&=R_{0}\symup{e}^{H_{0}\mleft(t-t_{0}\mright)},\tag{1}\\
\dot{R}\mleft(t\mright)&=H_{0}R\mleft(t\mright)=H_{0}R_{0}\symup{e}^{H_{0}\mleft(t-t_{0}\mright)}.\tag{2}\\
z\mleft(t\mright)&=\frac{R_{0}}{R\mleft(t\mright)}-1=\frac{1}{\symup{e}^{H_{0}\mleft(t-t_{0}\mright)}}-1.\tag{3}\\
\frac{\d{}z}{\d{}t}&=-H_{0}\symup{e}^{-H_{0}\mleft(t-t_{0}\mright)}=-H_{0}R_{0}/R\mleft(t\mright),\tag{4}\\
\frac{\d{}\omega}{\d{}z}&=\frac{\d{}\omega}{\d{}t}\frac{\d{}t}{\d{}z}=\frac{\symup{c}}{R\mleft(t\mright)}\frac{R\mleft(t\mright)}{H_{0}R_{0}}=\frac{\symup{c}}{H_{0}R_{0}}.\tag{5}\\
\omega&=\frac{\symup{c}z}{H_{0}R_{0}}.\tag{6}\\
k&=0, s\mleft(\omega\mright)=\omega=\frac{\symup{c}z}{H_{0}R_{0}}.\tag{7}
\end{align}
...
Comoving distance
\begin{equation}
\chi=\int_{t}^{t_{0}}\frac{\symup{c}}{R\mleft(t'\mright)}\d{}t'=\int_{0}^{z}\frac{\symup{c}}{H\mleft(z'\mright)}\d{}z'.\tag{1}
\end{equation}
In de Sitter space, $H\mleft(z\mright)=\dfrac{\dot{R}}{R}=H_{0}$,
\begin{equation}
...
\noindent\textcolor{purple}{(a)} Repeating Question 1, derive $\omega\mleft(z\mright)$ for the case of a spatially flat $\Omega=1$ matter-dominanted universe in which $R\mleft(t\mright)$ follows $R\mleft(t\mright)=R_{0}\mleft(\dfrac{3}{2}H_{0}t\mright)^{\frac{2}{3}}$. This is often called an `Einstein-de Sitter' universe. Add the angular size of a $\qty{1}{kpc}$ rod as a function of redshift to your plot from Q1 as well.

\hspace*{\fill}

\noindent\textcolor{purple}{Solution}
\begin{align}
R\mleft(t\mright)&=R_{0}\mleft(\frac{3}{2}H_{0}t\mright)^{\frac{2}{3}}.\tag{1}\\
z\mleft(t\mright)&=\frac{R_{0}}{R\mleft(t\mright)}-1=\mleft(\frac{3}{2}H_{0}t\mright)^{-\frac{2}{3}}-1.\tag{2}\\
\frac{\d{}z}{\d{}t}&=-H_{0}\mleft(\frac{3}{2}H_{0}t\mright)^{-\frac{5}{3}}=-H_{0}\mleft(\frac{R\mleft(t\mright)}{R_{0}}\mright)^{-\frac{5}{2}}.\tag{3}\\
\frac{\d{}\omega}{\d{}z}&=\frac{3\symup{c}t}{2R_{0}}=\frac{\symup{c}}{H_{0}R_{0}}\mleft(1+z\mright)^{-\frac{3}{2}}.\tag{4}\\
\omega&=\int_{0}^{z}\frac{\symup{c}}{H_{0}R_{0}}\mleft(1+z\mright)^{-\frac{3}{2}}\d{}z\notag\\
&=\frac{2\symup{c}}{H_{0}R_{0}}\mleft(1-\mleft(1+z\mright)^{-\frac{1}{2}}\mright).\tag{5}
\end{align}
Comoving distance
\begin{equation}
\chi=\int_{0}^{z}\frac{\symup{c}}{H\mleft(z'\mright)}\d{}z'.\tag{6}
\end{equation}
In Einstein-de Sitter universe,
\begin{align}
H\mleft(z\mright)&=\frac{\dot{R}}{R}=\frac{2}{3t}=H_{0}\mleft(1+z\mright)^{\frac{3}{2}}.\tag{7}\\
\chi&=\frac{\symup{c}}{H_{0}}\int_{0}^{z}\mleft(1+z'\mright)^{-\frac{3}{2}}\d{}z'\notag\\
&=\frac{2\symup{c}}{H_{0}}\mleft(1-\mleft(1+z\mright)^{-\frac{1}{2}}\mright).\tag{8}\\
\theta&=\frac{l}{\chi}\mleft(1+z\mright)=\frac{lH_{0}}{2\symup{c}}\frac{1+z}{1-\mleft(1+z\mright)^{-\frac{1}{2}}}.\tag{9}
\end{align}
...
&=0.001\times\frac{32\pi\times\mleft(3\times10^{5}\mright)^{3}}{3\times73.4^{3}}=\qty{2.288e9}{}.\tag{1}
\end{align}
...
\noindent\textcolor{purple}{(a)} Starting from the Friedmann equation, show that the critical density $\rho_{\mathrm{c}}$ (i.e. the density at which the Universe is flat with $\Omega=1, k=0$) equals
\begin{equation}
\rho_{\mathrm{c}}=\frac{3H\mleft(t\mright)^{2}}{8\pi\symup{G}}.\notag
\end{equation}
...
\rho_{\mathrm{cri}}&=\frac{3H\mleft(t\mright)^{2}}{8\pi\symup{G}}.\tag{2}
\end{align}
...
\noindent\textcolor{purple}{Solution}
\begin{align}
\rho_{\mathrm{cri}}&=\frac{3H\mleft(t\mright)^{2}}{8\pi\symup{G}}=\qty{9.2e-27}{kg\cdot{}m^{-3}}\tag{1}\\
...
\noindent\textcolor{purple}{(c)} Use your answer from question (b) to calculate the actual average baryon density of the Universe given that we know that $\Omega_{\mathrm{baryon}}=0.04$.

\hspace*{\fill}

\noindent\textcolor{purple}{Solution}
\begin{equation}
\rho_{\mathrm{b}}=0.04\rho_{\mathrm{cri}}=\qty{3.7e-28}{kg\cdot{}m^{-3}}.\tag{1}
\end{equation}
...
\noindent Hint: Use the fact that $\Omega_{\mathrm{baryon}}=0.04$ and use the Stefan-Boltzmann law, combined with average photon energy for a blackbody with $T=T_{\mathrm{CMB}}$.
...
Refer to question 3,
\begin{align}
\frac{n_{\gamma}}{n_{\mathrm{b}}}&=\frac{\qty{411}{}}{5.5\times10^{-6}\times0.04}\sim\qty{2e9}{}.\tag{3}
\end{align}
