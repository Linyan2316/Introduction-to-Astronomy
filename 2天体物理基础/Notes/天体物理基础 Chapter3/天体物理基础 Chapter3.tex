...
\begin{equation}
M_{\ce{H}\,\mathrm{\uppercase\expandafter{\romannumeral1}}}=2.3\times10^{5}\left(1+z\right)\left(\frac{D_{\mathrm{L}}}{\unit{Mpc}}\right)^{2}\left(\frac{F}{\unit{Jy\cdot km\cdot s^{-1}}}\right)\,\unit{M_{\odot}}.
\end{equation}
...
为简化模型，我们假设星系气体总是充分混合，恒星聚变产物瞬时返回星际介质，没有任何气体从星系逃离也没有任何气体流入。记时刻 $t$ 的气体质量为 $M_{\mathrm{g}}\left(t\right)$, 金属丰度为 $Z\left(t\right)$, 低质量恒星（寿命太长，产物在可预见的时标内不会返回到星际空间中）和大质量恒星遗迹（中子星、黑洞等，重元素被“锁”在其中也无法返回星际介质）的质量为 $M_{*}\left(t\right)$, 从恒星中返回星际介质的气体金属丰度为 $p$.

现在有 $\laplacian{}M_{\mathrm{g}}'\left(t\right)$ 的气体形成恒星，同时有 $\laplacian{}M_{\mathrm{g}}''\left(t\right)$ 的气体返回星际介质，因此 $\laplacian{}M_{\mathrm{g}}\left(t\right)=\laplacian{}M_{\mathrm{g}}''\left(t\right)-\laplacian{}M_{\mathrm{g}}'\left(t\right)$, 同时 $\laplacian{}M_{*}\left(t\right)=-\laplacian{}M_{\mathrm{g}}\left(t\right)$ 的气体留在恒星遗迹中。星际介质金属丰度的变化为
\begin{equation}
...
因此气体金属丰度随时间增加
\begin{equation}
Z\left(t\right)=Z_{0}+p\ln\left[\frac{M_{\mathrm{g}0}}{M_{\mathrm{g}}}\right].
\end{equation}
$M_{\mathrm{g}0}-M_{\mathrm{g}}$ 是已经转化成恒星的气体质量，因此金属丰度小于一定值 $Z$ 的恒星质量为
\begin{equation}
M\left(<Z\right)=M_{\mathrm{g}0}\left(1-\symup{e}^{-\frac{Z-Z_{0}}{p}}\right).
\end{equation}
金属丰度在 $Z$ 和 $Z+\laplacian{}Z$ 之间恒星的质量为
\begin{equation}
\frac{\d{}M_{*}}{\d{}Z}\laplacian{}Z\propto\exp\left\{-\left[Z-Z_{0}/p\right]\laplacian{}Z\right\}.
\end{equation}
...
星云质量足够高时，引力超过热运动提供的压力，就会引起星云坍缩。极限质量被称为金斯质量。动能 $\symup{K}=\dfrac{3}{2}N\symup{k}_{\mathrm{B}}T$, 势能 $\symup{U}\sim-\dfrac{3}{5}\dfrac{\symup{G}M^{2}}{R}, 2\symup{K}<\left\vert\symup{U}\right\vert$ 时星云坍缩。
...
\end{equation}
其中转动惯量 $I=\int\rho\left\vert\symbf{r}\right\vert^{2}\d{}^{3}x$, 动能 $T=\dfrac{1}{2}\int\rho\left\vert\symbf{u}\right\vert^{2}\d{}^{3}x$, 热能 $U=\dfrac{3}{2}\int n\symup{k}_{\mathrm{B}}T\d{}^{3}x$, 引力势能 $W=\dfrac{1}{2}\int\rho\phi_{g}\d{}^{3}x$, 磁能 $M=\dfrac{1}{8\pi}\int\left\vert\symbf{B}\right\vert^{2}\d{}^{3}x$.

气体云在引力坍缩时，$\dfrac{1}{2}\dfrac{\partial{}^{2}I}{\partial{}t^{2}}\approx-\dfrac{\symup{G}M^{2}}{R}$. 坍缩时标
\begin{equation}
t_{\mathrm{ff}}\sim\left(\frac{1}{4\pi\symup{G}\rho}\right)^{\frac{1}{2}}\sim5\times10^{5}\left(\frac{n}{\qty{e4}{cm^{-3}}}\right)^{-\frac{1}{2}}\,\unit{yr}.
\end{equation}
...
\begin{equation}
\xi\left(m\right)\d{}m=\xi_{0}m^{-\alpha}\d{}m,
\end{equation}
...
坍缩时标
\begin{equation}
\frac{\d{}^{2}r}{\d{}t^{2}}=-\frac{\symup{G}M_{r}}{r^{2}}\to t_{\mathrm{ff}}=\left(\frac{3\pi{}}{32\symup{G}\rho}\right)^{\frac{1}{2}}.
\end{equation}
