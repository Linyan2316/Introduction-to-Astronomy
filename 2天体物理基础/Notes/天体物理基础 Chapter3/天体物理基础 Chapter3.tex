...
\hline
成分 & 观测证据 & 温度 ($\unit{K}$) & 数密度 ($\symup{cm^{-3}}$) & 质量百分比\\
\cline{1-5}
...
\begin{equation}
M_{\ce{H}\,\symup{\uppercase\expandafter{\romannumeral1}}}=2.3\times10^{5}\left(1+z\right)\left(\frac{D_{\symup{L}}}{\unit{Mpc}}\right)^{2}\left(\frac{F}{\unit{Jy\cdot km\cdot s^{-1}}}\right)\,\unit{M_{\odot}}.
\end{equation}
...
为简化模型，我们假设星系气体总是充分混合，恒星聚变产物瞬时返回星际介质，没有任何气体从星系逃离也没有任何气体流入。记时刻 $t$ 的气体质量为 $M_{\symup{g}}\left(t\right)$, 金属丰度为 $Z\left(t\right)$, 低质量恒星（寿命太长，产物在可预见的时标内不会返回到星际空间中）和大质量恒星遗迹（中子星、黑洞等，重元素被“锁”在其中也无法返回星际介质）的质量为 $M_{*}\left(t\right)$, 从恒星中返回星际介质的气体金属丰度为 $p$.

现在有 $\laplacian{}M_{\symup{g}}'\left(t\right)$ 的气体形成恒星，同时有 $\laplacian{}M_{\symup{g}}''\left(t\right)$ 的气体返回星际介质，因此 $\laplacian{}M_{\symup{g}}\left(t\right)=\laplacian{}M_{\symup{g}}''\left(t\right)-\laplacian{}M_{\symup{g}}'\left(t\right)$, 同时 $\laplacian{}M_{*}\left(t\right)=-\laplacian{}M_{\symup{g}}\left(t\right)$ 的气体留在恒星遗迹中。星际介质金属丰度的变化为
\begin{equation}
\laplacian{}Z=Z'-Z\approx\frac{ZM_{\symup{g}}-Z\laplacian{}M_{\symup{g}}'+p\laplacian{}M_{\symup{g}}''}{M_{\symup{g}}}-Z=\frac{p\laplacian{}M_{\symup{g}}}{M_{\symup{g}}}.
\end{equation}

如果 $p$ 不依赖于金属丰度，那么
\begin{equation}
\frac{\laplacian{}Z}{p}=-\frac{\laplacian{}M_{\symup{g}}}{M_{\symup{g}}}.
\end{equation}
因此气体金属丰度随时间增加
\begin{equation}
Z\left(t\right)=Z_{0}+p\ln\left[\frac{M_{\symup{g}0}}{M_{\symup{g}}}\right].
\end{equation}
$M_{\symup{g}0}-M_{\symup{g}}$ 是已经转化成恒星的气体质量，因此金属丰度小于一定值 $Z$ 的恒星质量为
\begin{equation}
M\left(<Z\right)=M_{\symup{g}0}\left(1-\symup{e}^{-\frac{Z-Z_{0}}{p}}\right).
\end{equation}
...
星云质量足够高时，引力超过热运动提供的压力，就会引起星云坍缩。极限质量被称为金斯质量。动能 $\symup{K}=\dfrac{3}{2}Nk_{\symup{B}}T$, 势能 $\symup{U}\sim-\dfrac{3}{5}\dfrac{\symup{G}M^{2}}{R}, 2\symup{K}<\left\vert\symup{U}\right\vert$ 时星云坍缩。
...
\end{equation}
其中转动惯量 $I=\int\rho\left\vert\symbf{r}\right\vert^{2}\d{}^{3}x$, 动能 $T=\dfrac{1}{2}\int\rho\left\vert\symbf{u}\right\vert^{2}\d{}^{3}x$, 热能 $U=\dfrac{3}{2}\int nk_{\symup{B}}T\d{}^{3}x$, 引力势能 $W=\dfrac{1}{2}\int\rho\phi_{g}\d{}^{3}x$, 磁能 $M=\dfrac{1}{8\pi}\int\left\vert\symbf{B}\right\vert^{2}\d{}^{3}x$.

气体云在引力坍缩时，$\dfrac{1}{2}\dfrac{\partial{}^{2}I}{\partial{}t^{2}}\approx-\dfrac{\symup{G}M^{2}}{R}$. 坍缩时标
\begin{equation}
t_{\symup{ff}}\sim\left(\frac{1}{4\pi\symup{G}\rho}\right)^{\frac{1}{2}}\sim5\times10^{5}\left(\frac{n}{\qty{e4}{cm^{-3}}}\right)^{-\frac{1}{2}}\,\unit{yr}.
\end{equation}
...
坍缩时标
\begin{equation}
\frac{\d{}^{2}r}{\d{}t^{2}}=-\frac{\symup{G}M_{r}}{r^{2}}\to t_{\symup{ff}}=\left(\frac{3\pi{}}{32\symup{G}\rho}\right)^{\frac{1}{2}}.
\end{equation}
...
小质量恒星形成模型的放大版本，$M_{\symup{J}}>\qty{10}{M_{\odot}}$, 辐射加热或湍动、磁场主导内部压力。但是湍动作用未知，尚未观测到大质量恒星前核。

2. 竞争吸积

$M_{\symup{J}}\sim\qty{0.5}{M_{\odot}}$, 处于引力优势位置的原恒星更容易通过吸积周围的气体生长，但这样提供的恒星形成效率低。
