...
\begin{equation}
\frac{N_{r,k}}{N_{r,i}}=\frac{g_{r,k}}{g_{r,i}}\mathrm{e}^{-\frac{\varepsilon_{r,k}-\varepsilon_{r,i}}{k_{\text{B}}T}},\label{1.3.2}
\end{equation}
...
不管怎样库仑定律简化为$F=\dfrac{q_{1}q_{2}}{r^{2}}$,静电制中电荷单位也变为$\qty{1}{statC}=\qty{1}{dyne^{\frac{1}{2}}\cdot cm}$.考虑无量纲的真空光速数值$\mathrm{c}_{\text{n}}$, $\qty{1}{C}=\qty{2.99792458e9}{statC}$.静电制中电流基本单位$\qty{1}{statA}=\qty{1}{esu}=\qty{1}{statC\cdot{}s^{-1}}$.考虑$B=\dfrac{F}{IL}$我们还可以得到磁感应强度基本单位$\qty{1}{statT}=\qty{1}{dyne^{\frac{1}{2}}\cdot cm^{-2}\cdot s}=10^{-4}\mathrm{c}_{\text{n}}\,\unit{T}$.

而在静磁制中，电流的基本单位用以下方法定义：相距$\qty{1}{cm}$单位长度产生$\qty{2}{dyne}$相互作用力时的导线中的电流强度为$\qty{1}{emu}$.即$\qty{1}{emu}=\mathrm{c}\cdot\unit{dyne^{\frac{1}{2}}}=\qty{10}{A}$.此时得到静磁制下磁感应强度基本单位$\qty{1}{G}=\dfrac{1}{\mathrm{c}}\cdot\unit{dyne}^{\frac{1}{2}}\cdot\unit{cm}^{-1}=\qty{e-4}{T}=\dfrac1{\mathrm{c}_{\text{n}}}\,\unit{statT}$.

高斯单位制杂糅了静电制和静磁制，在电荷上使用静电制，磁场上使用静磁制，数值运算的时候可能会出现问题。如进行运动学运算的时候不能简单使用如$B=\qty{3}{\mu G}$的数据，而要转变成$B=\dfrac{3\mu}{\mathrm{c}_{\text{n}}}\,\unit{statT}$进行运算，否则静电制和静磁制会相互冲突。

但有意思的是，相对一般的 cgs 制，高斯单位制还有一个独立选择，定义$B_{\text{G}}=\mathrm{c}\cdot B_{\text{cgs}}$,使电场强度和磁感应强度有相同的量纲。这时候有些公式会变形，如洛伦兹力公式会变为$\boldsymbol{F}=q\dfrac{\boldsymbol{v}}{\mathrm{c}}\times\boldsymbol{B}_{\text{G}}$.考虑数值计算我们会惊奇地发现$B_{\text{G}}=3\mu\dfrac{\mathrm{c}}{\mathrm{c}_{\text{n}}}\,\unit{statT}=3\mu\cdot\unit{statT}\cdot\unit{cm\cdot{}s^{-1}}=3\mu\,\unit{dyne^{\frac{1}{2}}}\cdot\unit{cm}^{-1}$.即本质上$B_{\text{cgs}}=n\,\unit{G}$,运算上$B_{\text{G}}=n\,\unit{dyne^{\frac{1}{2}}\cdot cm^{-1}}$,高斯单位制成功统一了数值和量纲。
...
\end{equation}
由此我们就得到了每单位立体角辐射的功率。但需要注意的是，这是$t'$时刻发出的辐射$t$时刻的观测者处所接收到的部分，而我们更关心$t'$时刻粒子处的辐射分布。所以需要改写。$t$至$t+\mathrm{d}t$时间段内观测者在面元$\mathrm{d}\sigma$中接收的能量$\mathrm{d}P\left(t\right)\mathrm{d}t$是电荷在$t'$至$t'+\mathrm{d}t'$时间段内发出的，按能量守恒有
\begin{equation}
...
\begin{equation}
\langle{}P\rangle=\frac{4}{3}\sigma_{\text{T}}\mathrm{c}\gamma^{2}\beta^{2}U_{B}.\label{1.3.63}
\end{equation}
...
\begin{equation}
\frac{\mathrm{d}E_{{\color{blue}{\ce{e}}}}}{\mathrm{d}t}=\frac{4}{3}\gamma^{2}\sigma_{\text{T}}\mathrm{c}U_{\text{rad}}.\label{1.3.78}
\end{equation}
...
图\ref{同步加速辐射和 SSC 能谱示意图。}就展示了同步加速辐射和 SSC 能谱。可以发现左侧能谱左侧第一段受到同步加速辐射自吸收的影响。第二段的谱指数则与 SSC 一致。同步加速辐射第三段以$\mathrm{e}$指数下降，而右侧 SSC 能谱则在高能处截断。因此光谱中能观测到双峰分布，根据 (\ref{1.3.63})(\ref{1.3.78}) 式可知二者峰值比为
\begin{equation}
...
\begin{equation}
\frac{\ddot{a}}{a}=-\frac{4\pi\mathrm{G}}{3}\left(\rho\left(t\right)+3P\left(t\right)\right)+\frac{\Lambda}{3}.\label{1.6.11}
\end{equation}
...
\end{equation}
其中$n_{i}$是正整数，代表了在每个方向上驻波的半波数目，每个$\left(n_{x},n_{y},n_{z}\right)$所确定的点都代表一种允许的电磁波模式。注意到波数$k$与波长$\lambda$的关系为$k=\dfrac{2\pi{}}{\lambda}=\dfrac{2\pi{}\nu}{\mathrm{c}}$,我们也可以将上述边界条件用波数分量表示：
\begin{equation}
k^{2}=\sum_{i}k_{i}^{2},k_{i}=n_{i}\left(\frac{\pi}{L_{i}}\right),i=x,y,z.
\end{equation}
进而单位体积量子态的数目可以表示为
\begin{equation}
f\left(\nu\right)\mathrm{d}\nu=\frac{\mathrm{d}N}{V}=\frac{\mathrm{d}n_{x}\mathrm{d}n_{y}\mathrm{d}n_{z}}{V}=\frac{1}{\pi{}^{3}}\frac{L_{x}L_{y}L_{z}}{V}\mathrm{d}k_{x}\mathrm{d}k_{y}\mathrm{d}k_{z}=\frac{\mathrm{d}^{3}\vec{k}}{\pi{}^{3}}.
\end{equation}
在波数空间中，所有波数大小为$k$的点构成一个半径为$k$的球面，由于$n_{i}$都是正整数，因此只考虑八分之一球体体积，
\begin{equation}
f\left(\nu\right)\mathrm{d}\nu=\frac{1}{\pi^{3}}\frac{1}{8}4\pi{}k^{2}\mathrm{d}k=\frac{1}{\pi{}^{3}}\frac{1}{8}4\pi\cdot{}\frac{8\pi^{3}\nu^{2}}{\mathrm{c}^{3}}\mathrm{d}\nu=\frac{4\pi{}\nu^{2}}{\mathrm{c}^{3}}\mathrm{d}\nu.
\end{equation}
...
\begin{align}
Z&=\left(\sum_{n_{1}=1,2,\cdots}\mathrm{e}^{-\beta n_{1}\varepsilon_{1}}\right)\left(\sum_{n_{2}=1,2,\cdots}\mathrm{e}^{-\beta n_{2}\varepsilon_{2}}\right)\cdots\\
&=\left(\frac{1}{1-\mathrm{e}^{-\beta\varepsilon_{1}}}\right) \left(\frac{1}{1-\mathrm{e}^{-\beta\varepsilon_{2}}}\right)\cdots.
\end{align}
第$i$个模式上的平均光子数可通过对巨配分函数的对数求导得到：
\begin{equation}
\langle{}n_{i}\rangle=-\frac{1}{\beta}\frac{\partial\ln Z}{\partial\varepsilon_{i}}=\frac{1}{\mathrm{e}^{\beta\varepsilon_{i}}-1}.
\end{equation}
综上我们便求得了黑体辐射的公式：
\begin{equation}
\varepsilon\left(\nu\right)\mathrm{d}\nu=\frac{8\pi{}\nu^{2}}{\mathrm{c}^{3}}\cdot\frac{1}{\mathrm{e}^{\frac{h\nu}{k_{\text{B}}T}}-1}\cdot{}h\nu\mathrm{d}\nu.
...
汤姆孙散射可看作平面波驱动电子偶极震荡
\begin{align}
\mathrm{d}I&=\frac{1}{4\pi\mathrm{c}^{3}}\left(\ddot{\boldsymbol{d}}\times\boldsymbol{n}\right)^{2}\mathrm{d}\Omega=\frac{{\color{blue}{\ce{e}}}^{4}}{4\pi\mathrm{c}^{3}m_{0}^{2}}\left(\boldsymbol{E}\times\boldsymbol{n}\right)^{2}\mathrm{d}\Omega,\\
\mathrm{d}\sigma\left(\theta,\phi\right)&=\sigma\left(\theta,\phi\right)\mathrm{d}\Omega=\frac{\mathrm{d}I\left(\theta,\phi\right)}{S}=\left[\frac{{\color{blue}{\ce{e}}}^{2}}{m_{0}\mathrm{c}^{2}}\right]^{2}\sin^{2}\theta\mathrm{d}\Omega,
\end{align}
其中$S$是 Poynting 矢量。总散射截面为
\begin{equation}
\sigma_{\text{T}}=\frac{8\pi}{3}\left(\frac{{\color{blue}{\ce{e}}}^{2}}{m_{0}\mathrm{c}^{2}}\right)^{2}=\qty{6.65e-25}{cm^{-2}}.
\end{equation}
