...
\begin{align}
\left(\frac{\dot a}{a}\right)^{2}&=\frac{8\pi\mathrm{G}}{3}\sum_{i}\rho_{i}-\frac{k}{a^{2}},\\
\frac{\ddot a}{a}&=-\frac{4\pi\mathrm{G}}{3}\sum_{i}\left(\rho_{i}+3\mathcal{P}_{i}\right).
\end{align}
对第一式求导，得到
\begin{equation}
2\dot a\ddot a=\frac{8\pi\mathrm{G}}{3}\left(\dot\rho a^{2}+2\rho\dot aa\right).
\end{equation}
结合第二式得到
\begin{equation}
...
哈勃常数变化
\begin{align}
\Omega_{\text{r}}=\frac{8\pi\mathrm{G}}{3H_{0}^{2}}\rho_{\text{r}},\Omega_{\text{m}}=\frac{8\pi\mathrm{G}}{3H_{0}^{2}}\rho_{\text{m}},\Omega_{\Lambda}=\frac{8\pi\mathrm{G}}{3H_{0}^{2}}\rho_{\Lambda},\Omega_{\text{k}}=-\frac{k}{a^{2}H_{0}^{2}}.\\
H\left(z\right)=H_{0}\sqrt{\Omega_{\text{r0}}\left(1+z\right)^{4}+\Omega_{\text{m0}}\left(1+z\right)^{3}+\Omega_{\text{k0}}\left(1+z\right)^{2}+\Omega_{\Lambda0}}.
\end{align}

宇宙年龄
\begin{equation}
t_{0}=\int_{0}^{a_{0}}\frac{\mathrm{d}a}{\dot a}=\int_{0}^{\infty}\frac{\mathrm{d}z}{\left(1+z\right)H\left(z\right)}.
\end{equation}

视界
\begin{equation}
d_{\text{H}}=a_{0}\int_{0}^{t_{0}}\frac{\mathrm{c}\mathrm{d}t}{a\left(t\right)}.
\end{equation}
...
\section{CMB}
Acoustic oscillations：Recombination 以前声速达到$\dfrac{\mathrm{c}}{\sqrt{3}}$，直到共动声学世界在$R_{\text{s}}\sim150\,\mathrm{Mpc}$冻结。Recombination 时 Jeans 长度比视界尺度大得多，因此我们预计在所有亚视界尺度上都会有流体振荡，包括 Recombination 时刚好进入视界的尺度。视界尺度上流体的动力学将转化为宇宙微波背景辐射功率谱中的第一个声学峰。
...
\begin{align}
\delta{}\left(\vec{x}\right)&=\frac{1}{\left(2\pi\right)^{\frac32}}\int\delta{}\left(\vec{k}\right)e^{-i\boldsymbol{k}\cdot\boldsymbol{x}}\mathrm{d}^{3}\vec{k}.\\
\delta{}\left(\vec{k}\right)&=\frac{1}{\left(2\pi\right)^{\frac32}}\int\delta{}\left(\vec{x}\right)e^{i\boldsymbol{k}\cdot\boldsymbol{x}}\mathrm{d}^{3}\vec{x}.\\
\end{align}

定义物质扰动满足
\begin{align}
&\frac{\delta{}\rho}{\rho_{0}}=\delta{}\left(t\right)e^{i\boldsymbol{k}\cdot\boldsymbol{r}}.\\
&\frac{\mathrm{d}^{2}\delta{}}{\mathrm{d}t^{2}}+2\left(\frac{\dot a}{a}\right)\frac{\mathrm{d}\delta{}}{\mathrm{d}t}-\delta{}\left(4\pi\mathrm{G}\rho_{0}-k^{2}c_{\text{s}}^{2}\right)=0.
\end{align}

对于小扰动，忽略声波，考虑纯物质宇宙
\begin{equation}
4\pi\mathrm{G}\rho_{0}=\frac{2}{3t^{2}},\frac{\dot a}{a}=\frac{2}{3t},
\end{equation}
得到
\begin{equation}
\frac{\mathrm{d}^{2}\delta{}}{\mathrm{d}t^{2}}+\frac{4}{3t}\frac{\mathrm{d}\delta{}}{\mathrm{d}t}-\frac{2}{3t^{2}}\delta{}=0.
\end{equation}
取$\delta{}=at^{n}$，代入得到
\begin{equation}
...
曲率为主的宇宙，
\begin{equation}
\frac{\mathrm{d}^{2}\delta{}}{\mathrm{d}t^{2}}+\frac{2}{t}\frac{\mathrm{d}\delta{}}{\mathrm{d}t}=0.
\end{equation}
解只有$n=0,-1$，即扰动不能增长。
