...
\begin{equation}
E=\int_{0}^{\infty}\frac{L}{4\pi r^{2}}\times n\times4\pi r^{2}\d{}r\to\infty.
\end{equation}
...
由于太阳的运动，CMB 相对我们有一多普勒偶极不对称性，相对背景量级在 $10^{-3}$. 扣除此效应和星系前景辐射，CMB 各向异性 $\dfrac{\laplacian{}T}{T}$ 仅有 $10^{-5}$ 量级的变化。
...
\begin{equation}
\frac{a}{a_{0}}=\mleft(\frac{32\pi\symup{G}}{3\symup{c}^{2}}U_{0}\mright)^{\frac{1}{4}}\tau^{\frac{1}{2}},
\end{equation}
因此
\begin{equation}
\tau=\mleft(\frac{32\pi\symup{G}}{3\symup{c}^{2}}U\mright)^{-\frac{1}{2}}\sim T_{10}^{-2}\,\unit{s}.
\end{equation}
...
偶极不对称性：太阳相对哈勃流运动引起的涨落，太阳朝向和背向温度变化约 $10^{-3}$, $\laplacian{}T=\qty{3.353}{mK}$
...
宇宙线是十分高能的。高速粒子 ($\qty{e20}{eV}$) 与 CMB 光子反应，因此能量高于一定程度的宇宙线并不存在，观测中会发现 Greisen-Zatsepin-Kuzmin cutoff, 大约在 $\qty{e20}{eV}$.

早期宇宙引力倾向于使重子成团，光子则倾向于消除各向异性。二者的相互作用导致等离子体在引力势中振荡。这种振荡产生的温度和密度变化被记录下来。宇宙膨胀，光子平均自由程增大，微波背景辐射小范围衰减，因此振动频谱记录了温度涨落和宇宙结构。

光子持续和电子发生康普顿散射，使 $T_{\mathrm{matter}}\sim{}T_{\mathrm{rad}}$, 直到 $z\sim1500$ 温度小于 $\qty{13.6}{eV}$. 引入电离度
\begin{equation}
\chi\mleft(z\mright)=\qty{3.4e-3}{}\frac{\mleft(\Omega_{\mathrm{m}}h_{70}^{2}\mright)^{\frac{1}{2}}}{\Omega_{\mathrm{b}}h_{70}^{2}}\mleft(\frac{z}{1000}\mright)^{12.75}=0.04\mleft(\frac{z}{1000}\mright)^{12.75},
\end{equation}
计算光深
\begin{equation}
\tau\mleft(z\mright)=\int_{0}^{z}n_{{\color{blue}{\ce{e}}}}\mleft(z\mright)\chi\mleft(z\mright)\sigma_{\mathrm{T}}\frac{\d{}l}{\d{}z}\d{}z.
\end{equation}
$n_{{\color{blue}{\ce{e}}}}$ 和 $\dfrac{\d{}l}{\d{}z}$ 依赖于 $\Omega_{\mathrm{b}},\Omega_{\mathrm{m}}$ 和 $h_{70}^{2}$, 因此积分结果最后只和 $z$ 有关
\begin{equation}
\tau\mleft(z\mright)=0.37\mleft(\frac{z}{1000}\mright)^{14.25}.
\end{equation}
散射概率为 $P=\symup{e}^{-\tau}$, 最终散射概率密度分布为
\begin{equation}
P'\mleft(z\mright)\d{}z=-\frac{\d{}P}{\d{}z}\d{}z=\symup{e}^{-\tau}\frac{\d\tau}{\d{}z}\d{}z.
\end{equation}
...
\draw (180, 10) node [anchor=north west][inner sep=0.75pt]   [align=left] {$R_{\mathrm{eq}}$};
\draw (360, 10) node [anchor=north west][inner sep=0.75pt]   [align=left] {$R_{\mathrm{rec}}$};
...
我们可以想办法描述这种振荡的演化。定义扰动场 $\delta\mleft(\vec{x}\mright)$, 进行 Fourier 变换
\begin{align}
\delta\mleft(\vec{x}\mright)&=\frac{1}{\mleft(2\pi\mright)^{\frac{3}{2}}}\int\delta\mleft(\vec{k}\mright)\symup{e}^{-i\symbf{k}\cdot\symbf{x}}\d{}^{3}\vec{k}.\\
\delta\mleft(\vec{k}\mright)&=\frac{1}{\mleft(2\pi\mright)^{\frac{3}{2}}}\int\delta\mleft(\vec{x}\mright)\symup{e}^{i\symbf{k}\cdot\symbf{x}}\d{}^{3}\vec{x}.
\end{align}
定义物质扰动满足
\begin{align}
&\frac{\delta\rho}{\rho_{0}}=\delta\mleft(t\mright)\symup{e}^{i\symbf{k}\cdot\symbf{r}}.\\
&\frac{\d{}^{2}\delta}{\d{}t^{2}}+2\mleft(\frac{\dot a}{a}\mright)\frac{\d\delta}{\d{}t}-\delta\mleft(4\pi\symup{G}\rho_{0}-k^{2}\symup{c}_{\mathrm{s}}^{2}\mright)=0.\label{6.5.4}
\end{align}
...
(\ref{6.5.4}) 式变为
\begin{equation}
\frac{\d{}^{2}\delta}{\d{}t^{2}}+\frac{4}{3t}\frac{\d\delta}{\d{}t}-\frac{2}{3t^{2}}\delta=0.
\end{equation}
...
\begin{equation}
\delta_{k}=\delta_{k, 0}\mleft(\frac{t}{t_{0}}\mright)^{\frac{2}{3}}=\delta_{k, 0}\mleft(\frac{a}{a_{0}}\mright).
\end{equation}

曲率为主的宇宙，
\begin{equation}
\frac{\d{}^{2}\delta}{\d{}t^{2}}+\frac{2}{t}\frac{\d\delta}{\d{}t}=0.
\end{equation}
解只有 $n=0,-1$, 即扰动不能增长。

辐射主导时，
\begin{equation}
\frac{\d{}^{2}\delta}{\d{}t^{2}}+2\mleft(\frac{\dot a}{a}\mright)\frac{\d\delta}{\d{}t}-\delta\mleft(\frac{32}{3}\pi\symup{G}\rho-k^{2}\symup{c}_{\mathrm{s}}^{2}\mright)=0.
\end{equation}
考虑到 $a\propto\tau^{\frac{1}{2}}$,
\begin{equation}
\delta_{k}=\delta_{0}\mleft(\frac{a}{a_{0}}\mright)^{2}.
\end{equation}
