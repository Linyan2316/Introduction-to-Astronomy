...
光子持续和电子发生康普顿散射，使 $T_{\symup{matter}}\sim{}T_{\symup{rad}}$, 直到 $z\sim1500$ 温度小于 $\qty{13.6}{eV}$. 引入电离度
\begin{equation}
\chi\left(z\right)=\qty{3.4e-3}{}\frac{\left(\Omega_{\symup{m}}h_{70}^{2}\right)^{\frac{1}{2}}}{\Omega_{\symup{b}}h_{70}^{2}}\left(\frac{z}{1000}\right)^{12.75}=0.04\left(\frac{z}{1000}\right)^{12.75},
\end{equation}
计算光深
\begin{equation}
\tau\left(z\right)=\int_{0}^{z}n_{{\color{blue}{\ce{e}}}}\left(z\right)\chi\left(z\right)\sigma_{\symup{T}}\frac{\d{}l}{\d{}z}\d{}z.
\end{equation}
$n_{{\color{blue}{\ce{e}}}}$ 和 $\dfrac{\d{}l}{\d{}z}$ 依赖于 $\Omega_{\symup{b}},\Omega_{\symup{m}}$ 和 $h_{70}^{2}$, 因此积分结果最后只和 $z$ 有关
\begin{equation}
...
\draw (180, 10) node [anchor=north west][inner sep=0.75pt]   [align=left] {$R_{\symup{eq}}$};
\draw (360, 10) node [anchor=north west][inner sep=0.75pt]   [align=left] {$R_{\symup{rec}}$};
...
&\frac{\d{}^{2}\delta}{\d{}t^{2}}+2\left(\frac{\dot a}{a}\right)\frac{\d\delta}{\d{}t}-\delta\left(4\pi\symup{G}\rho_{0}-k^{2}\symup{c}_{\symup{s}}^{2}\right)=0.\label{6.5.4}
\end{align}
...
辐射主导时，
\begin{equation}
\frac{\d{}^{2}\delta}{\d{}t^{2}}+2\left(\frac{\dot a}{a}\right)\frac{\d\delta}{\d{}t}-\delta\left(\frac{32}{3}\pi\symup{G}\rho-k^{2}\symup{c}_{\symup{s}}^{2}\right)=0.
\end{equation}
