...
\begin{equation}
\d{}M_{r}=4\pi r^{2}\rho\left(r\right)\d{}r
\end{equation}
和流体静力学方程
\begin{equation}
\frac{\d{}P}{\d{}r}=-\frac{\symup{G}M_{r}\rho\left(r\right)}{r^{2}}\label{2.1.2}
\end{equation}
就不赘述了。
...
首先 Maxwell 速度分布为
\begin{align}
f\left(v\right)\d{}v&=\left(\frac{m}{2\pi\symup{k}_{\mathrm{B}}T}\right)^{\frac{3}{2}}\exp\left(-\frac{mv^{2}}{2\symup{k}_{\mathrm{B}}T}\right)4\pi v^{2}\d{}v,\\
f\left(E\right)\d{}E&=\frac{2}{\sqrt{\pi}}\frac{E^{\frac{1}{2}}}{\left(\symup{k}_{\mathrm{B}}T\right)^{\frac{3}{2}}\exp\left(-\dfrac{E}{\symup{k}_{\mathrm{B}}T}\right)}\d{}E.
\end{align}
...
\begin{align}
\sigma\left(E\right)&=\frac{S\left(E\right)}{E}\exp\left[-\frac{E_{\mathrm{G}}^{\frac{1}{2}}}{E^{\frac{1}{2}}}\right],\\
E_{\mathrm{G}}&=\left[\frac{1}{2\varepsilon_{0}\hbar}\left(\frac{m}{2}\right)^{\frac{1}{2}}Z_{1}Z_{2}{\color{blue}{\ce{e}}}^{2}\right]^{2},
\end{align}
结合 Maxwell 速度分布就能得到反应速率：
\begin{align}
r&=n_{1}n_{2}\int_{0}^{\infty}\sigma\left(E\right)vf\left(E\right)\d{}E\notag\\
&=n_{1}n_{2}\frac{2^{\frac{3}{2}}}{\left(\pi m\right)^{\frac{1}{2}}}\frac{1}{\left(\symup{k}_{\mathrm{B}}T\right)^{\frac{3}{2}}}\int_{0}^{\infty}S\left(E\right)\exp\left(-\frac{E_{\mathrm{G}}^{\frac{1}{2}}}{E^{\frac{1}{2}}}\right)\exp\left(-\frac{E}{\symup{k}_{\mathrm{B}}T}\right)\d{}E.
\end{align}
观察上式可知在某个能量处反应速率最大，也就是 Gamow Peak:
\begin{equation}
E_{0}=\left[\left(\frac{m}{2}\right)^{\frac{1}{2}}\frac{Z_{1}Z_{2}{\color{blue}{\ce{e}}}^{2}\symup{k}_{\mathrm{B}}T}{4\varepsilon_{0}\hbar}\right]^{\frac{2}{3}}=\left(\frac{E_{\mathrm{G}}^{\frac{1}{2}}\symup{k}_{\mathrm{B}}T}{2}\right)^{\frac{2}{3}}.
\end{equation}
看表达式可知 $E_{0}$ 是热能 $\symup{k}_{\mathrm{B}}T$ 和 $E_{\mathrm{G}}$ 按照 $2: 1$ 的几何平均。

接着计算能量产生效率，首先一次反应释放的能量为
\begin{equation}
\laplacian\varepsilon=\left(A_{1}+A_{2}\right)f\left(A_{12}\right)-A_{1}f\left(A_{1}\right)-A_{2}f\left(A_{2}\right)+E_{0}.
\end{equation}
将 $S\left(E_{0}\right)$ 从反应速率表达式的积分中提出来，得到
\begin{equation}
r\propto{}n_{1}n_{2}\frac{S\left(E_{0}\right)}{T^{\frac{2}{3}}}\exp\left[-3\left(\frac{{\color{blue}{\ce{e}}}^{4}}{32\varepsilon_{0}^{2}\symup{k}_{\mathrm{B}}\hbar^{2}}\frac{mZ_{1}^{2}Z_{2}^{2}}{T}\right)^{\frac{1}{3}}\right].
\end{equation}

于是我们可以定义聚变效率 $C$, 一般用实验而不是计算确定。最后可借此得到单位体积的能量产生率 $\varepsilon$
\begin{align}
r\laplacian\varepsilon&=\rho\varepsilon.\\
&\varepsilon=C\rho X_{1}X_{2}\frac{1}{T^{\frac{2}{3}}}\exp\left[-3\left(\frac{{\color{blue}{\ce{e}}}^{4}}{32\varepsilon_{0}^{2}\symup{k}_{\mathrm{B}}\hbar^{2}}\frac{mZ_{1}^{2}Z_{2}^{2}}{T}\right)^{\frac{1}{3}}\right].
\end{align}
...
\begin{equation}
\int_{0}^{R}\frac{\d{}P}{\d{}r}4\pi{}r^{3}\d{}r=\int_{0}^{R}\left(-\frac{\symup{G}M_{r}}{r^{2}}\rho\right)4\pi{}r^{3}\d{}r,
\end{equation}
利用分部积分得到
\begin{equation}
-\int_{0}^{R}3P4\pi{}r^{2}\d{}r=\int_{0}^{R}\left(-\frac{\symup{G}M_{r}}{r}\right)4\pi{}r^{2}\rho\d{}r.
\end{equation}
...
\begin{equation}
P_{\mathrm{g}}=n_{{\color{blue}{\ce{e}}}}\symup{k}_{\mathrm{B}}T+\sum_{i}n_{i}\symup{k}_{\mathrm{B}}T=\sum_{i}\left(Z_{i}+1\right)n_{i}\symup{k}_{\mathrm{B}}T=\sum_{i}\left(1+Z_{i}\right)\frac{\rho\chi_{i}\symup{k}_{\mathrm{B}}T}{A_{i}m_{\ce{H}}}=\frac{\rho\symup{k}_{\mathrm{B}}T}{\mu m_{\ce{H}}}.
\end{equation}
因此相对分子量为
\begin{equation}
...
\begin{equation}
P=\frac{1}{3}\int vpf\left(p\right)4\pi p^{2}\d{}p.\label{2.1.37}
\end{equation}
...
因此费米动量可以表示为
\begin{equation}
p_{\mathrm{F}}=\left(\frac{3h^{3}\rho}{8\pi\mu_{{\color{blue}{\ce{e}}}}m_{\ce{H}}}\right)^{\frac{1}{3}}.
\end{equation}
...
代入辐射转移方程得到
\begin{align}
I\left(0,\mu\right)&=\int_{0}^{\infty}\frac{S}{\mu}\symup{e}^{\left(-\frac{t}{\mu}\right)}\d{}t=\frac{3F}{4\pi}\left(\mu+q\right),\\
F&=2\pi\int_{0}^{1}I\left(0,\mu\right)\mu\d\mu=\frac{3F}{2}\left(\frac{1}{3}+\frac{q}{2}\right).\\
q&=\frac{2}{3}.
\end{align}
再根据斯特藩玻尔兹曼方程得到
\begin{align}
...
接下来我们研究能量在恒星内部的传输。首先建立能量守恒方程。记单位时间内通过半径为 $r$ 的球面的能量为 $L\left(r\right)$, 单位物质在单位时间产生的能量（来自核反应和引力势能转换）为 $\varepsilon\left(r\right)$, 那么半径为 $r$, 厚度为 $\d{}r$ 的球壳两侧的能量差为
\begin{equation}
\d{}L_{r}=L\left(r+\d{}r\right)-L\left(r\right)=\varepsilon\d{}M_{r}=4\pi{}r^{2}\rho\varepsilon\d{}r.
\end{equation}
...
\begin{equation}
\frac{\d{}T}{\d{}r}=\left.\frac{\d{}T}{\d{}r}\right\vert_{\mathrm{rad}}+\left.\frac{\d{}T}{\d{}r}\right\vert_{\mathrm{conv}}.
\end{equation}
...
\begin{equation}
F=-\frac{\symup{c}}{\alpha_{\mathrm{R}}}\frac{\d{}}{\d{}z}\left(\frac{a_{\mathrm{B}}}{3}T^{4}\right).
\end{equation}
...
\left.\frac{\d{}T}{\d{}r}\right\vert_{\mathrm{ad}}&=\left(1-\frac{1}{\gamma}\right)\frac{T}{P}\frac{\d{}P}{\d{}r}=-\left(1-\frac{1}{\gamma}\right)\frac{\mu m_{\ce{H}}}{\symup{k}_{\mathrm{B}}}\frac{\symup{G}M_{r}}{r^{2}}.
\end{align}
我们认为 $\left\vert\dfrac{\d{}T}{\d{}r}\right\vert_{\mathrm{actual}}<\left\vert\dfrac{\d{}T}{\d{}r}\right\vert_{\mathrm{ad}}$ 的是辐射区，反之是对流区。
...
\begin{align}
\frac{\symup{G}M_{r}\rho\d{}A\d{}r}{r^{2}}&=\left(P\left(r\right)-P\left(r+\d{}r\right)\right)\d{}A=-\d{}P\left(r\right)\d{}A,\\
\frac{\d{}P}{\d{}r}&=-\frac{\symup{G}M_{r}\rho}{r^{2}}.\\
\frac{\d{}}{\d{}r}\left(\frac{r^{2}}{\rho}\frac{\d{}P}{\d{}r}\right)&=-\symup{G}\frac{\d{}M_{r}}{\d{}r}=-\symup{G}\left(4\pi r^{2}\rho\right).\\
\frac{1}{r^{2}}\frac{\d{}}{\d{}r}\left(\frac{r^{2}}{\rho}\frac{\d{}P}{\d{}r}\right)&=-4\pi\symup{G}\rho.
\end{align}
...
如此一来压力引力平衡方程可以改写为
\begin{equation}
\left(\frac{n+1}{n}\right)\frac{K}{r^{2}}\frac{\d{}}{\d{}r}\left(r^{2}\rho^{\frac{1}{n}-1}\frac{\d\rho}{\d{}r}\right)=-4\pi\symup{G}\rho.\label{2.1.33}
\end{equation}
接着作代换
\begin{align}
\rho\left(r\right)&=\rho_{\mathrm{c}}\left[\theta_{n}\left(r\right)\right]^{n},\\
r&=a_{n}\xi\left(r\right),\\
a_{n}&=\left[\frac{\left(n+1\right)K}{4\pi\symup{G}}\rho_{\mathrm{c}}^{\frac{1}{n}-1}\right]^{\frac{1}{2}},\label{2.1.86}
\end{align}
其中 $\rho_{\mathrm{c}}$ 代表恒星中心的密度，如此一来 (\ref{2.1.33}) 式可以改写为 Lane-Emden 方程的形式：
\begin{equation}
\frac{1}{\xi^{2}}\frac{\d{}}{\d\xi}\left(\xi^{2}\frac{\d\theta_{n}}{\d\xi}\right)=-\theta_{n}^{n}.
\end{equation}
上式仅在 $n=0, 1, 5$ 时有解析解：
\begin{align}
...
\begin{equation}
t_{n}=\frac{E}{L}=\frac{\eta\laplacian{}M\symup{c}^{2}}{L}\approx0.7\%\times\frac{0.1\, M_{\odot}\symup{c}^{2}}{L}\approx\qty{e10}{yr}\frac{M}{\unit{M_{\odot}}}\left(\frac{L}{\unit{L_{\odot}}}\right)^{-1}.
\end{equation}

热时标，恒星辐射完自身内能所需的时间，或者光子从恒星内部到达表面的时间（根据位力定理，内能是引力能的 $\dfrac{1}{2}$)
\begin{equation}
t_{\mathrm{the}}=0.5\frac{\symup{G}M^{2}}{RL}\approx\qty{2e7}{yr}\left(\frac{M}{\unit{M_{\odot}}}\right)^{2}\left(\frac{R}{\unit{R_{\odot}}}\right)^{-1}\left(\frac{L}{\unit{L_{\odot}}}\right)^{-1}.
\end{equation}

动力学时标，如果恒星内部压力突然消失，引力作用下恒星坍缩的时间
\begin{equation}
t_{\mathrm{dyn}}=\frac{R}{V}\approx\sqrt\frac{R^{3}}{\symup{G}M}\approx\qty{27}{min}\left(\frac{R}{\unit{R_{\odot}}}\right)^{\frac{3}{2}}\left(\frac{M}{\unit{M_{\odot}}}\right)^{-\frac{1}{2}}.
\end{equation}
...
现在考虑一个小扰动
\begin{equation}
\frac{P_{0}+P_{1}}{P_{0}}=\left(\frac{\rho_{0}+\rho_{1}}{\rho_{0}}\right)^{\gamma}, P_{1}=\symup{c}_{\mathrm{s}}^{2}\rho_{1},
\end{equation}
连续性方程变为
\begin{equation}
...
&\left(\frac{\partial{}^{2}}{\partial{}t^{2}}-\symup{c}_{\mathrm{s}}^{2}\nabla^{2}-4\pi\symup{G}\rho_{0}\right)\rho_{1}=0.
\end{align}
...
同时有金斯质量：
\begin{equation}
M_{\mathrm{J}}=\frac{4\pi}{3}\rho\left(\frac{\lambda_{\mathrm{J}}}{2}\right)^{3}\sim 2M_{\odot}\cdot\left(\frac{\symup{c}_{\mathrm{s}}}{\qty{0.2}{km\cdot{}s^{-1}}}\right)^{3}\left(\frac{n}{\qty{e3}{cm^{-3}}}\right)^{-\frac{1}{2}}.
\end{equation}
质量大于这个值的分子云才能坍缩。
...
\begin{equation}
\frac{\partial{}^{2}\Delta_{k}}{\partial{}t^{2}}=\left(2\pi\symup{G}\Sigma_{0}k-\Omega^{2}-k^{2}\symup{c}_{\mathrm{s}}^{2}\right)\Delta_{k}.
\end{equation}
Toomre Q parameter
\begin{equation}
...
\end{equation}
对给定粒子数，$d$ 取 $2N^{-\frac{2}{3}}\left(m_{0}\alpha_{\mathrm{G}}\right)^{-1}$ 时 $T$ 有最大值，其中 $\alpha_{\mathrm{G}}=\dfrac{\symup{G}m_{\ce{p}}^{2}}{\hbar\symup{c}}$:
\begin{equation}
...
可以得到临界粒子数密度为
\begin{equation}
N=\eta^{\frac{3}{4}}\left(\dfrac{m_{\ce{p}}}{m_{0}}\right)^{\frac{3}{4}}\left(\frac{\alpha}{\alpha_{\mathrm{G}}}\right)^{\frac{3}{2}},
\end{equation}
假定 $\eta=0.1$, 临界质量为
\begin{equation}
...
\symbf{\Phi}&=-\frac{1}{1+q}\frac{\symup{G}M_{\mathrm{total}}}{\left\vert\symbf{r}-\left(-\dfrac{q}{1+q}a, 0, 0\right)\right\vert{}}-\frac{q}{1+q}\frac{\symup{G}M_{\mathrm{total}}}{\left\vert\symbf{r}-\left(\dfrac{1}{1+q}a, 0, 0\right)\right\vert{}}-\frac{1}{2}\frac{\symup{G}M_{\mathrm{total}}}{a^{3}}r^{2}.
\end{align}
...
随着费米动量越来越高，$E_{F}=\sqrt{p_{\mathrm{F}}^{2}\symup{c}^{2}+m_{{\color{blue}{\ce{e}}}}^{2}\symup{c}^{4}}>\left(m_{\ce{n}}-m_{\ce{p}}-m_{{\color{blue}{\ce{e}}}}\right)\symup{c}^{2}$, 从而生成中子。临界动量和密度为
\begin{align}
p_{\mathrm{F}}&=m_{{\color{blue}{\ce{e}}}}\symup{c}\left[\left(\frac{m_{\ce{n}}-m_{\ce{p}}}{m_{{\color{blue}{\ce{e}}}}}\right)^{2}-1\right]^{\frac{1}{2}}.\\
\rho&\sim\qty{1.2e10}{kg\cdot m^{-3}}.
\end{align}
等密度达到 $\qty{e18}{kg\cdot m^{-3}}$ 的量级，最终产生半径 $\qty{10}{km}$ 的中子星。中子星的特征质量为 $\qty{1.4}{M_{\odot}}$.

中子星质量估计：考虑质子德布罗意波长和普朗克质量 $m_{\mathrm{planck}}=\sqrt{\dfrac{\hbar\symup{c}}{\symup{G}}}$, 估计得到半径上限 $R_{\max}\sim\dfrac{m_{\mathrm{planck}}}{m_{\ce{p}}}\times\lambda_{n}\sim\qty{10}{km}$, 粒子数上限 $N\sim\left(\dfrac{m_{\mathrm{planck}}}{m_{\ce{p}}}\right)^{3}\sim2.2\times10^{57}$, 质量上限 $M_{\max}\sim Nm_{\ce{p}}\sim M_{\odot}$, 密度上限 $\rho_{\max}\sim\dfrac{m_{\ce{p}}}{\lambda_{n}}\sim\qty{e18}{kg\cdot m^{-3}}$.
...
\begin{equation}
U_{\mathrm{g}}\sim\frac{\symup{G}M^{2}}{R}=2.7\times10^{53}\,\unit{erg}\left(\frac{M}{\unit{M_{\odot}}}\right)^{2}\left(\frac{R}{\qty{10}{km}}\right)^{-1},
\end{equation}
然后是转动动能
\begin{align}
U_{\mathrm{rot}}&\sim M\Omega^{2}R^{2}\sim7.9\times10^{52}\,\unit{erg}\left(\frac{M}{\unit{M_{\odot}}}\right)\left(\frac{R}{\qty{10}{km}}\right)^{2}\left(\frac{\nu}{\qty{1000}{Hz}}\right)^{2}.\\
\frac{U_{\mathrm{rot}}}{U_{\mathrm{g}}}&=0.3\left(\frac{M}{\unit{M_{\odot}}}\right)^{-1}\left(\frac{R}{\qty{10}{km}}\right)^{3}\left(\frac{\nu}{\qty{1000}{Hz}}\right)^{2}.
\end{align}
接着是磁能
\begin{align}
...
\frac{U_{\mathrm{B}}}{U_{\mathrm{g}}}&\sim3\times10^{-13}\left(\frac{M}{\qty{1.4}{M_{\odot}}}\right)^{-2}\left(\frac{R}{\qty{10}{km}}\right)^{4}\left(\frac{B}{\qty{e12}{G}}\right)^{2}.
\end{align}

与通常恒星相比中子星的温度很高，但相对于中子星物质的高密度而言，它可以非常好地用零温近似描述，因此费米能为
\begin{align}
\varepsilon_{\mathrm{Feimi}}&=\frac{P_{\mathrm{F}}^{2}}{2m_{\ce{n}}}=\frac{h^{2}}{2m_{\ce{n}}}\left(\frac{3n}{8\pi}\right)^{\frac{2}{3}}=\qty{30}{MeV}\left(\frac{\rho}{\qty{e14}{g\cdot cm^{-3}}}\right)^{\frac{2}{3}}\gg\symup{k}_{\mathrm{B}}T.\\
E_{\mathrm{F}}&=\qty{1.64e53}{erg}\left(\frac{M}{\unit{M_{\odot}}}\right)^{\frac{5}{3}}\left(\frac{R}{\qty{10}{km}}\right)^{-2}.
\end{align}
...
\begin{equation}
\d{}s^{2}=\left(-1+\frac{2\symup{G}M}{r\symup{c}^{2}}\right)\symup{c}^{2}\d{}t^{2}+\frac{1}{1-\dfrac{2\symup{G}M}{r\symup{c}^{2}}}\d{}r^{2}+r^{2}\left(\d\theta^{2}-\sin^{2}{\theta}\d\phi^{2}\right).
\end{equation}
...
\begin{equation}
r_{\mathrm{h}}=\left(\frac{R_{\mathrm{s}}}{2}\right)\left[m+\sqrt{m^{2}-a^{2}}\right].
\end{equation}
能层为
\begin{equation}
r_{\mathrm{st}}=\left(\frac{R_{\mathrm{s}}}2\right)\left[m+\sqrt{m^{2}-a^{2}\cos^{2}{\theta}}\right].
\end{equation}
...
假设行星是黑体，能量全部来自太阳，得到
\begin{align}
&\frac{L_{\odot}}{4\pi R^{2}_{\odot\to\mathrm{planet}}}\pi R_{\mathrm{planet}}^{2}\times\left(1-a\right)=4\pi R_{\mathrm{planet}}^{2}\sigma_{\mathrm{SB}}T_{\mathrm{planet}}^{4}.\\
&T_{\mathrm{eq}}=\left(1-a\right)^{\frac{1}{4}}\sqrt{\frac{R_{\odot}}{2R_{\odot\to\mathrm{planet}}}}T_{\odot}.
\end{align}
可借此估计行星温度，其中 $a$ 是反照率。
...
M_{\mathrm{p}}\ll M_{1}, M_{\mathrm{p}}=v_{1}M_{1}^{\frac{2}{3}}\left(\frac{T}{2\pi\symup{G}}\right)^{\frac{1}{3}}.
\end{align}

还要考虑倾角
\begin{equation}
M_{\mathrm{p}}\sin{i}=v_{1,\mathrm{obs}}M_{1}^{\frac{2}{3}}\left(\frac{T}{2\pi\symup{G}}\right)^{\frac{1}{3}}.
\end{equation}
...
\theta_{\mathrm{E}}&=\left(\frac{4\symup{G}M}{\symup{c}^{2}}\frac{D_{\mathrm{sl}}}{D_{\mathrm{lo}}D_{\mathrm{so}}}\right)^{\frac{1}{2}}.
\end{align}
...
透镜会放大相，放大率满足
\begin{align}
a_{\pm}=\frac{\theta_{\pm}}{\beta}\frac{\d\theta_{\pm}}{\d\beta}=\frac{\theta_{\pm}}{\beta}\left[1\pm\frac{\beta}{\left(\beta^{2}+4\theta_{\mathrm{E}}^{2}\right)^{\frac{1}{2}}}\right].\\
a_{\mathrm{total}}=a_{+}+a_{-}=\frac{u^{2}+2}{u\left(u^{2}+4\right)^{\frac{1}{2}}}, u=\frac{\beta}{\theta_{\mathrm{E}}}.
\end{align}

对于系外源我们忽视 $\beta$ 的变化，我们会考虑恒星穿越透镜区域
\begin{equation}
\beta\left(t\right)=\left[\beta_{0}^{2}+\frac{v^{2}}{D_{\mathrm{lo}}^{2}}\left(t-t_{0}\right)^{2}\right]^{\frac{1}{2}}.
\end{equation}
因此会产生光变曲线。光变时标
\begin{equation}
...
\begin{equation}
\frac{\tau_{\mathrm{p}}}{\tau_{\mathrm{star}}}=\frac{\theta_{\mathrm{p}}}{\theta_{\mathrm{star}}}=\left(\frac{M_{\mathrm{p}}}{M_{\mathrm{star}}}\right)^{\frac{1}{2}}.
\end{equation}
...
引力差距
\begin{equation}
\laplacian{}F=\frac{\symup{G}Mm}{r^{2}}\left(\cos{\phi}\left(1+\frac{2R}{r}\cos{\theta}\right)-1\right)\hat{i}-\frac{\symup{G}Mm}{r^{2}}\left(1+\frac{2R}{r}\cos{\theta}\right)\sin{\phi}\hat{j}.
\end{equation}
其中 $\phi$ 是被引力体表面某点与引力体中心连线，相对二引力体中心连线的夹角。该式表明，被引力体赤道延伸，两极压缩。

潮汐力会引起天体解体
\begin{equation}
\frac{2\symup{G}M_{\mathrm{p}}R_{\mathrm{m}}}{r^{3}}>\frac{\symup{G}M_{\mathrm{m}}}{R_{\mathrm{m}}^{2}}, r<2^{\frac{1}{3}}\left(\frac{M_{\mathrm{p}}}{M_{\mathrm{m}}}\right)^{\frac{1}{3}}R_{\mathrm{m}}.
\end{equation}
