...
太阳为地球提供了大量的光和热，一个很自然的问题便出现在科学家们的脑海中：恒星的能量来源是什么？有人 (Herschel, 1837) 认为来自氢氧合成水过程中所释放的化学能，有人 (Mayer, 1846) 认为来自小行星撞击恒星所提供的能量。一个看起来比较靠谱的猜想是引力能的释放 (Waterson, Helmholtz, Kelvin, 1850), 恒星不断向外辐射能量，内能降低导致压强下降，此时为了平衡引力恒星就会收缩，引力能在此过程中转化为内能。那么这个过程足以维持数亿年的辐射吗？假设恒星密度均匀，恒星的引力能为
\begin{align}
...
到了 1920 年代，卢瑟福等人已经测定了原子核的质量，同时相对论质能方程的提出让人们意识到恒星的能量可能来自于热核反应。那么，恒星内部的环境满足进行核聚变的条件吗？我们先估计一下电荷数最小的氢原子聚变所需克服的经典库仑势垒。取质子直径$r\sim\qty{e-15}{m}$,所需的温度为
\begin{align}
...
\end{equation}
因此太阳平均温度可达$T=\dfrac{\mu m_{\ce{H}}\overline{P}}{k_{\text{B}}\rho}\sim\qty{5e6}{K}$,核心温度可达$\qty{1.5e7}{K}$,仅凭借引力能达到的高温就足以支持氢的热核反应。
...
观察分布可知在某个能量处核反应速率最大，也就是 Gamow Peak:
\begin{equation}
...
\ce{^{13}N &->^{13}C +e+}+\nu_{\ce{e}},\quad\text{$\beta$-decay $\tau_{\frac12}\sim\qty{10}{min}$},\\
\ce{^{13}C +^{1}H &->^{14}N}+\gamma,\\
\ce{^{14}N +^{1}H &->^{15}O}+\gamma,\\
\ce{^{15}O &->^{15}N +e+}+\nu_{\ce{e}},\quad\text{$\beta$-decay $\tau_{\frac12}\sim\qty{2}{min}$},\\
...
\subsection{恒星的演化}
恒星通过核反应产生能量，维持热平衡和流体静力学平衡。在此过程中恒星内部的化学组成发生变化。将新的化学组成作为初始条件重新代入方程组求解，得到恒星在时间$\symup{d}t$后的结构。依次类推，可以求得恒星的结构随时间的变化，即恒星的演化。
...
\end{align}
上式中，如果引力可被忽略，就是声波方程。如果压力可被忽略，$\rho_{1}\propto{}\exp\left[\sqrt{4\pi\symup{G}\rho_{0}}t\right]$.通过三维傅立叶变换可以看出波数和密度分布。

(\ref{2.2.11}) 式又称流体静力学色散关系：
\begin{equation}
\omega^{2}=k^{2}c_{\text{s}}^{2}-4\pi\symup{G}\rho_{0},
\end{equation}
其中$c_{\text{s}}$是声速。要让波发生演化而不是形成驻波，需要$\omega$是复数。于是我们引入金斯波长
\begin{equation}
...
\begin{equation}
E_{\text{gravity}}\sim\frac{\symup{G}M^{2}}{R}\sim\frac{\symup{G}N^{2}m_{\ce{p}}^{2}}{R},
\end{equation}
分子间的结合能为
\begin{equation}
...
亥姆霍兹开尔文时标约一千万年，恒星需要快速丢失引力势能。可能在原恒星外围有尘埃包络，吸收光学厚高能光子转化成光学薄红外光子辐射出去。

太阳仅保留了$10^{-5}$的初始角动量，可能是依靠粘滞力矩将角动量往外运，将质量往里运。

磁场提供磁压力，阻碍坍缩。中性材料的两极扩散带走磁性物质转移磁场。

观测表明，$\qty{e-3}{M_{\odot}}$行星数量较多，随后逐渐下降，$\qty{e-2}{M_{\odot}}$逐渐观测不到，然后$\qty{e-1}{M_{\odot}}$大小的行星又开始出现。表明行星形成有两种机制。一种是大质量坍缩（所以小质量少因为不足以坍缩），一种是从尘埃开始成长（所以小质量多）。

尘埃加气体盘的组合只有数百万年的寿命。

类地行星更重，靠近太阳，多为硅、铁、镍、氧。类木行星易挥发成分更多，如氢、冰、氦。

太阳系行星形成三步走：

1. 尘埃生长$\unit{\mu m}\to\unit{cm}$,

2. 行星胚胎$\unit{cm}\to\unit{km}$,

3. 大质量恒星$\unit{km}\to\qty{e4}{km}$.

首先，不同物质会在不同温度凝结到尘埃表面。一般当$T<\dfrac{\lambda}{R}$才会发生。当尘埃越来越重，声速逐渐降低，开始主要受旋转和引力影响，有个最大质量
\begin{align}
\omega^{2}=\Omega^{2}-2\pi\symup{G}\Sigma k,\lambda<4\pi^{2}\symup{G}\Sigma\Omega^{-2}.\\
M_{\max}\sim\Sigma\lambda_{\max}^{2}\sim16\pi^{4}\symup{G}^{2}\Sigma^{3}\Omega^{-4}.
\end{align}

如果声速不可忽略不计，那么主要在大尺度发挥作用
\begin{equation}
\lambda>\frac{c_{\text{s}}^{2}}{\symup{G}\Sigma}\sim\qty{e7}{AU}.
\end{equation}
还可以刻画阈值声速
\begin{equation}
k^{2}c_{\text{s}}^{2}\sim\Omega^{2}.
\end{equation}
声速受到速度和温度限制
\begin{equation}
c_{\text{s}}\sim\left(\frac{3k_{\text{B}}T}{m}\right)^{\frac{1}{2}},
\end{equation}
因此由缓慢移动的大量粒子组成的气体将比原子/分子盘更易受重力不稳定影响。对于$\lambda\sim\qty{e4}{km},\\c_{\text{s}}\sim\qty{e-7}{km\cdot{}s^{-1}}$，引力坍缩时标大概一百万年。

最后一步，$\unit{km}$级别行星胚胎碰撞成大质量行星。类地行星的大气是由来自“雪线”之外的富含挥发性物质的行星胚胎撞击形成的。大于五倍地球质量的行星还能吸积大量的氢和氦气体。

大质量行星清空空隙导致旋臂密度变化，产生扭矩，大质量行星能迁移到离太阳更近的地方。
...
恒星内部的$\ce{H}$燃烧通过$\symup{CNO}$循环进行，内部温度更高，辐射压对维持恒星的力学平衡起更大的作用，主序寿命更短。氦核不再简并。更重元素的燃烧可以平稳进行。核心区产生的能量主要以对流的方式向外传递。
...
(horizontal branch) & 壳层氢燃烧，壳层半径下降，温度上升 & \\
(HB) & & \\
\cline{1-3}
...
\Phi\left(r\right)&=-\frac{\symup{G}M_{1}}{\left\vert{}\symbf{r}-\symbf{r}_{1}\right\vert{}}-\frac{\symup{G}M_{2}}{\left\vert{}\symbf{r}-\symbf{r}_{2}\right\vert{}}-\frac{1}{2}\left(\symbf{\omega}\times\symbf{r}\right)^{2}.
\end{align}
...
对于非相对论性 ($v_{{\color{blue}{\ce{e}}}}\ll\symup{c}$) 电子，$P_{{\color{blue}{\ce{e}}}}\propto\rho^{\frac{5}{3}}$,对于相对论性电子，$P_{{\color{blue}{\ce{e}}}}\propto\rho^{\frac{4}{3}}$.
...
新星的观测特征为在数天至数星期内亮度增加$7\text{\textendash}16$等，然后缓慢下降，经几个月或几年回复到原先的状态。辐射主要在光学和紫外波段，爆发时的能量释放率$\sim10^{45}\text{\textendash}\qty{e46}{erg\cdot{}s^{-1}}$,抛射约$10^{-5}\text{\textendash}\qty{e-3}{M_{\odot}}$的物质。
...
\end{equation}
轨道倾角$i$似乎并非随机分布。考虑立体角积分方式，实际上是正弦分布。
...
\begin{equation}
\beta(t)=\left[\beta_{0}^{2}+\frac{v^{2}}{D_{\text{lo}}^{2}}\left(t-t_{0}\right)^{2}\right]^{\frac{1}{2}}.
\end{equation}
因此会产生光变曲线。光变时标
\begin{equation}
...
潮汐力会引起天体解体
\begin{equation}
\frac{2\symup{G}M_{\text{p}}R_{\text{m}}}{r^{3}}>\frac{\symup{G}M_{\text{m}}}{R_{\text{m}}^{2}},r<2^{\frac{1}{3}}\left(\frac{M_{\text{p}}}{M_{\text{m}}}\right)^{\frac13}R_{\text{m}}.
\end{equation}
其中$R_{\text{m}}$是卫星半径。
