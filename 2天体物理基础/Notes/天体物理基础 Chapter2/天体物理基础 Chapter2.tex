...
\ce{4{}^{1}H ->{}^{4}He +2e^{+}}+2\nu_{\ce{e}}+\text{energy}.
\end{equation}
其中$\nu_{\ce{e}}$是电子中微子。此过程中释放的能量为$E=\left(4m_{\ce{H}}-m_{\ce{He}}\right)\symup{c}^{2}=\qty{4e-5}{erg}$,燃烧效率为$0.7\%$,即以质量计有$0.7\%$的初始燃料完全转化为能量。
...
稍作整理并修正下正负号便是所需要的方程
\begin{equation}
\frac{\symup{d}T}{\symup{d}r}=-\frac{3}{4a_{\text{B}}\symup{c}}\frac{\kappa\rho}{T^{3}}\frac{L_{r}}{4\pi r^{2}}.
\end{equation}
可见不透明度$\kappa$会影响辐射能传输的效率。不透明度下降，散逸的能量增多，导致核心区温度下降，压强下降，恒星收缩，密度上升，进而导致不透明度上升，产生$\kappa$机制振荡。

太阳仅有$\qty{6000}{K}$，氢原子难以激发，何来不透明度？原因在于氢负离子的电子容易被激发。 
...
a_{n}&=\left[\frac{\left(n+1\right)K}{4\pi\symup{G}}\rho_{\text{c}}^{\frac{1}{n}-1}\right]^{\frac{1}{2}},
\end{align}
...
流体静力学色散关系：
\begin{equation}
\omega^{2}=k^{2}v_{\text{s}}^{2}-4\pi\symup{G}\rho_{0},
\end{equation}
其中$v_{\text{s}}$是声速。要让波发生演化而不是形成驻波，需要$\omega$是复数。于是我们引入金斯波长
\begin{equation}
\lambda_{\text{J}}=\frac{h}{p_{\text{J}}}=\frac{2\pi}{k_{\text{J}}}=\sqrt{\frac{\pi v_{\text{s}}^{2}}{\symup{G}\rho_{0}}},
\end{equation}
同时有金斯质量：
\begin{equation}
...
核坍缩超新星极大光度可达$10^{7}\text{\textendash}\qty{e10}{L_{\odot}}$，总爆发能$\qty{e53}{erg}$, 其中$99\%$以上以中微子形式释放；放射性衰变释放能量$\sim\qty{e49}{erg}$, 可以产生时标几个月的可见光辐射；抛射物质$\sim\qty{10}{M_{\odot}}$, 速度$\sim\qty{3000}{km\cdot{}s^{-1}}$, 合计总动能$\sim\qty{e51}{erg}$.
...
白矮星基本参数：
质量$\sim0.2\text{\textendash}\qty{1.3}{M_{\odot}}$, 半径$\sim5\times10^{8}\text{\textendash}\qty{e9}{cm}$, 密度$\sim10^{5}\text{\textendash}\qty{e7}{g\cdot cm^{-3}}$, 绝对星等$M_{\text{V}}\sim8\text{\textendash}16$, 有效温度$\sim5\times10^{3}\text{\textendash}\qty{4e4}{K}$,自转周期$\ge\qty{10}{s}$. 外层包裹着厚度约$\qty{50}{km}$，主要由氢组成的大气。
